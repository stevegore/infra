...
- **Tailscale** — `tailscaled.service`
  ...
  - **Known fault: MagicDNS dies host-wide, intermittently.** NetworkManager is set
    ...
    "only tailnet names are broken" — every `*-pico` / `vault-oke` / `macbook`
    lookup fails with `no such host on 192.168.4.1:53`. Observed 2026-07-31 and
    again 2026-08-01 immediately after a tailscaled DNS reconfig, and on
    2026-08-02 after the device renames triggered another reconfig.
