...
| Vaultwarden `/data` | **`emptyDir` (ephemeral)** | All meaningful state is in MySQL; everything else in `/data` is unused or regenerable. The one file that must persist, `rsa_key.pem`, is restored from Vault by an init container rather than by a PVC. See §7.1. |
...
- **`/data` is ephemeral.** Vaultwarden's only durable surface in OKE is MySQL; the container's `/data` is an `emptyDir`. Walking through what's normally in there (see the upstream backup wiki) against our actual usage:
  ...
  - `rsa_key.pem` — **the exception; now restored from Vault, not regenerated.** The upstream wiki's "deletion only forces re-login" made this look cheap, and the original reasoning here priced it as a one-off cost "on a stateless rebuild". That was wrong in practice: a rollout happens on *any* values change, not just a rebuild, so from 2026-06-06 to 2026-07-26 every deploy silently logged every device out of every Bitwarden client. Since 2026-07-26 the key is held as the `rsa_key` field of `kv/vaultwarden/config` and copied into `/data` by the `restore-rsa-key` init container, so sessions now survive restarts, node loss and cluster rebuilds alike. The whole file is 1675 bytes, so this costs nothing against the block-volume budget the rest of this section is protecting. See `apps/vaultwarden/values.yaml` (`jwtKey`) for the seed/rotate procedure.
...
- **Hourly sync** via systemd timer on pico: `mysqldump --single-transaction --databases vaultwarden` from HeatWave Free → convert with `mysql2sqlite` (or equivalent) → swap the pico sqlite file atomically while the container is briefly stopped (`docker stop vaultwarden && mv new.sqlite data/db.sqlite3 && docker start vaultwarden`). No `/data` sync — pico's standby keeps its own RSA key, so clients still re-auth when they fail over to `bw2` and again when they come back. That is now a deliberate `bw2`-only cost rather than a fact of life: the OKE primary's key is pinned in Vault as of 2026-07-26 (§7.1), so restarts there no longer log anyone out. Seeding pico with the same key would make failover seamless too, at the cost of putting the primary's signing key on a second host — not worth it while `bw2` is a break-glass path. There's no other meaningful file state to mirror. Total downtime per sync: ~5 sec on the standby; the primary on OKE isn't touched.
...
- [x] API endpoint NSG — TCP 6443 from the home IP (`var.home_ip_cidr`) only. Bump `home_ip_cidr` in `terraform/terraform.tfvars` (gitignored) if/when the home IP changes (see `terraform/oke-networking.tf`).
...
- [x] Point a **test** subdomain (e.g. `oke-test.stevegore.au`) at the reserved IP in Cloudflare (after NLB IP is fixed) — verify Caddy + cert issuance work end-to-end before touching real records. *(Record deleted 2026-07-25 once the migration was long done.)*
...
| Vaultwarden `/data` PVC? | **No — treat `/data` as ephemeral (`emptyDir`), and put the one durable file in Vault.** Every file in `/data` is either superseded by MySQL (`db.sqlite3`), unused (`sends/`, `config.json`) or regenerable (`icon_cache/`). The lone historical attachment was deleted 2026-05-24 to make this hold cleanly. `rsa_key.pem` was originally lumped in as "only triggers a one-time re-auth", which understated it — losing it on every rollout logged out every device — so since 2026-07-26 it is stored in Vault and restored by an init container. Still saves 50 GB of block-volume budget, still keeps the RWO single-writer constraint out of the failover story, and is *better* than a PVC would have been: a Vault-held key also survives node loss and cluster rebuilds. |
